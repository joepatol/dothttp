...
The parser SHALL recognise lines of the form `@name = value` and store them as `Variable` structs in `HttpFile.variables`. Variable interpolation (substitution of `{{name}}` references) is explicitly out of scope.
